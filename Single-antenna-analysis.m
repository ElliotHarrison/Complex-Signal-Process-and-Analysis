...
% Create upperbound of data analysis iterations and initialize random data modulation band.    
    for i = 1 : ct
        s = randsrc(1, 1, [-1 1]);
        h1 = randn(1,1); % Create one transmit and receive channel (antenna) for data passthrough.
        h2 = randn(1,1);
        n = randn(1,1)/sqrt(SNR); % Generate random AWGN noise.
     
% Receive data observations on signal plus noise.
        y1 = h1 * s+ h2 * s + n;
        y2 = abs(h1) * s + abs(h2) * s + n;
     
% Detect data using prior estimates of observation output.
        prio_est1 =y1/(h1+h2); % first step of detection 
        prio_est2 = y2/(abs(h1)  + abs(h2));
